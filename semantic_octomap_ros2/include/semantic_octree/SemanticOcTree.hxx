...
namespace octomap {
    ...
    SemanticOcTree<SEMANTICS>::SemanticOcTree(double resolution)
            : OccupancyOcTreeBase<SemanticOcTreeNode<SEMANTICS> >(resolution),
    ...
    {
        // LE format: le[0]=run_length, le[1..NUM_SEMANTICS]=class logodds, le[NUM_SEMANTICS+1]=others
        KeyRay* keyray = &(this->keyrays.at(0));
        if (this->computeRayKeys(origin, end, *keyray))
        {
            auto push_unknown = [](semantic_octomap_interfaces::msg::LE& le) {
                for (int i = 0; i < NUM_SEMANTICS + 1; i++)
                    le.le.push_back(-0.1);
            };
            auto push_uniform = [](semantic_octomap_interfaces::msg::LE& le, float l) {
                for (int i = 0; i < NUM_SEMANTICS + 1; i++)
                    le.le.push_back(l);
            };
            auto push_semantics = [](semantic_octomap_interfaces::msg::LE& le, const SEMANTICS& sem) {
                for (int i = 0; i < NUM_SEMANTICS; i++)
                    le.le.push_back(sem.data[i].logOdds);
                le.le.push_back(sem.others);
            };

            semantic_octomap_interfaces::msg::LE le_msg_prev;
            le_msg_prev.le.push_back(1);
            SemanticOcTreeNode<SEMANTICS>* prevNode = this->search(*(keyray->begin()));
            if (!prevNode) {
                push_unknown(le_msg_prev);
            } else if (!prevNode->isSemanticsSet()) {
                push_uniform(le_msg_prev, prevNode->getLogOdds() - log(static_cast<float>(NUM_SEMANTICS + 1)));
            } else {
                push_semantics(le_msg_prev, prevNode->getSemantics());
            }
                ...
                if (!currNode && !prevNode) {
                    ...
                    if (*currNode == *prevNode) {
                        ...
                        if (!currNode->isSemanticsSet())
                            push_uniform(le_msg_prev, currNode->getLogOdds() - log(static_cast<float>(NUM_SEMANTICS + 1)));
                        else
                            push_semantics(le_msg_prev, currNode->getSemantics());
                        prevNode = currNode;
                    }
                } else {
                    rayRLE_msg.le_list.push_back(le_msg_prev);
