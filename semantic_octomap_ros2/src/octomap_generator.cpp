...
void OctomapGenerator<CLOUD, OCTREE>::insertPointCloud(const pcl::PCLPointCloud2::Ptr& cloud, const Eigen::Matrix4f& sensorToWorld)
{
    ...
    // lazy_eval=false → updates inner-node occupancy immediately. Required so
    // get_ray_RLE sees a consistent tree right after insert (lazy_eval=true
    // left inner nodes stale and crashed the next query).
    if(raycast_cloud.size() > 0)
        octomap_.insertPointCloud(raycast_cloud, origin, raycast_range_, false);

    // Belt-and-suspenders: explicitly recompute inner occupancy and colors
    // after each insert. Slightly more expensive but rules out tree-consistency
    // bugs causing segfaults in get_ray_RLE.
    if(endpoint_count > 0)
        octomap_.updateInnerOccupancy();
}

template<class CLOUD, class OCTREE>
bool OctomapGenerator<CLOUD, OCTREE>::get_ray_RLE(const octomap::point3d& origin, const octomap::point3d& end,
